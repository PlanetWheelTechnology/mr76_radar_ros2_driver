...
bool Mr76Driver::ExtractDetections(const can_msgs::msg::Frame &frame,
                                   RadarData &radar_out) {
  switch (frame.id) {
    case MessageID::kIdObjStatus:
      if (true == buffer_manager_.IsReached()) {  // successfull scene extraction
        ...
        RCLCPP_WARN(rclcpp::get_logger("Mr76Driver"), "BUFFER WARN,Skipping Scene");
        data_parsed.total_targets = 0;
        data_parsed.targets.clear();
        buffer_manager_.Reset();
        buffer_manager_.SetPeak(frame.data[0]);
        // RCLCPP_WARN(rclcpp::get_logger("Mr76Driver"), "Buffer reset, peak set to %d", frame.data[0]);
      }
      break;
...
int16_t Mr76Driver::ParseTotalObjects(const can_msgs::msg::Frame &frame) {
  return (int16_t)frame.data[0];
}

// parses the target information
TargetData Mr76Driver::ParseTargetInfo(const can_msgs::msg::Frame &frame) {
  int16_t data_stream[8];  // stream of extracted bits defining data
  TargetData parsed_output;
  ...
  parsed_output.id = data_stream[0] * 1;
  parsed_output.dist_long = data_stream[1] * 0.2 - 500; 
  parsed_output.dist_lat = data_stream[2] * 0.2 - 204.6;
  parsed_output.vel_rel_long = data_stream[3] * 0.25 - 128.0;
  parsed_output.vel_rel_lat = data_stream[4] * 0.25 - 64.0;
  parsed_output.dynamic_prop = data_stream[6] * 1;
  parsed_output.rcs = data_stream[7] * 0.5 - 64.0;
...
void BufferHandle::SetPeak(int32_t new_peak) { peak_ = new_peak; }

void BufferHandle::UpdateCounter() { ++counter_; }

uint32_t BufferHandle::GetCounter() { return counter_; }

// bool BufferHandle::IsReached() { return (counter_ == peak_ ? true : false); }
bool BufferHandle::IsReached() { return (counter_ <= peak_ ? true : false); }

bool BufferHandle::IsOverFlow() { return (counter_ > peak_ ? true : false); }

bool BufferHandle::IsUnderFlow() { return (counter_ < peak_ ? true : false); }
