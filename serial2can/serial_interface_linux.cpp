 ...
 * @author David Hu (hmd@ldrobot.com)
...
bool SerialInterfaceLinux::Open(std::string &port_name, uint32_t com_baudrate) {
  ...
  if (-1 == com_handle_) {
#ifdef USE_ROS2LOGGER
    RCLCPP_ERROR(rclcpp::get_logger("usv_controller_driver"), "open error:%s", strerror(errno));
#else
...
#ifdef USE_ROS2LOGGER
    RCLCPP_ERROR(rclcpp::get_logger("usv_controller_driver"), "TCGETS2 first error:%s", strerror(errno));
#else
...
#ifdef USE_ROS2LOGGER
    RCLCPP_ERROR(rclcpp::get_logger("usv_controller_driver"), "TCSETS2 error:%s", strerror(errno));
#else
  ...
  if (ioctl(com_handle_, _IOC(_IOC_READ, 'T', 0x2A, sizeof(struct asmtermios::termios2)), &options)) {
#ifdef USE_ROS2LOGGER
    RCLCPP_ERROR(rclcpp::get_logger("usv_controller_driver"), "TCGETS2 second error:%s", strerror(errno));
#else
...
#ifdef USE_ROS2LOGGER
  RCLCPP_INFO(rclcpp::get_logger("usv_controller_driver"), "actual baudRate reported:%d", options.c_ospeed);
#else
  LOG_INFO("actual baudRate reported:%d", options.c_ospeed);
#endif
